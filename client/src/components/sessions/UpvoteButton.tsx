...
interface UpvoteButtonProps {
  ...
  sessionId: string;
  onUpvoteSuccess: (questionId: string, newUpvotes: number) => void;
}
...
export function UpvoteButton({
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        console.error("Failed to parse upvoted questions", e);
      }
    }
  }, [questionId]);
  ...
  const handleUpvote = async () => {
    ...
    try {
      ...
      if (onUpvoteSuccess) {
        onUpvoteSuccess(questionId, updated.upvotes);
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <ThumbsUp
        className={`h-3.5 w-3.5 ${hasUpvoted ? "fill-black text-black" : ""}`}
      />
